...
class DetalleProducto extends Component {   
 ...
 componentDidMount () { 
   fetch('/id')
     ...
     .then(data => {
       this.setState({ producto: data.producto})
     })
     .catch(err => {
     })
 }
 render(){
   var mis_productos = this.state.productos.map(i => { 
     return <div className='detalleProductos'>{},{}</div>
   })
     return (    
             {mis_productos}
        ); 
  ...
  render() {
        return (    
          <div className='producto'>  
            ...
            <h4 className='DescripcionProducto'>Características principales: Impresionante calidad de fotografía : Toma tus mejores fotografías con el conocimiento en cámaras de Sony. Xperia XA1 cuenta con una cámara de 23 MP con sensor Exmor RS™ para dispositivos móviles y lente F2.0. Más pixeles, más luz e imágenes inmejorables.
            ...
            Más memoria : Agrega hasta 256 GB de memoria con una tarjeta Micro SD para guardar tus fotografías, videos y música.</h4>
        </div>
        ); 
    }
}
